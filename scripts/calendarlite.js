...
;(function($){
    ...
    $.fn.calendarLite.draw = function(opts) {
        ...
        var table = $('<table cellspacing="1" class="calendar"></table>');
        ...
        for (var j = 0; j < dates.length; j++) {
            ...
            f = false;
            for (var i = 0; i < arrlinks.length; i++) if(arrlinks[i][2] == date && arrlinks[i][0] == year && arrlinks[i][1] == month) f = true;
            if(f) line.push('<td' + cl + '><b><a href="' + href + '" rel="' + rel + '">' + date + '</a></b></td>');
            else line.push('<td' + cl + '>' + date + '</td>');
            if (dates[j].getDay() == 0) {
                if (line.length < 7) {
                    var ln = line.length;
                    var pad = [];
